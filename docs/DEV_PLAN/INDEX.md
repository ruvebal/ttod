...
| Y | Agentic dirtree Unix tidy (“Dear Tree”) — explicit pack-vs-leaf criteria; README legend + Cathedral/Bazaar Tri-Anchor epigraph; optional layout moves; then `lao-tzu-tao-compose` → ttod-bridge proposals for collection `dear_tree`. **Does not** invent `~/src/deviac/agentic`, absorb studio skills, or mutate `ttod.yml` without human accept. | mini-cascade Y0→Y4; branch only after Y0+Y1 auth | **Y0–Y3 DONE, Y4 agent work DONE (2026-09-20)** — branch `agentic/dear-tree-tidy`, PR pending. Plan audited (9 inconsistencies), legend README + `ide-mcp/PACK.md`, layout unchanged by evidence (flat packs; legibility test with a negative control), compose lens measured (2 of 3 lanes dead). **Open for a named human:** accept/reject the 5 staged `blackbox` proposals (their printed accept commands are stale — verified recipe in `PHASE-Y4-REPORT.md`), and decide whether `collections.dear_tree` is ever created (no CLI verb; hand-edit forbidden). | [`PHASE-Y-AGENTIC-DIRTREE-UNIX/INDEX.md`](PHASE-Y-AGENTIC-DIRTREE-UNIX/INDEX.md) · [`PHASE-Y-AGENTIC-DIRTREE-UNIX-CASCADE.md`](PHASE-Y-AGENTIC-DIRTREE-UNIX/PHASE-Y-AGENTIC-DIRTREE-UNIX-CASCADE.md) |
