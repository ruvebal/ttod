...
## Proposed programme (Phase S DONE through S4; Phase R R0–R5 DONE, R7 PARTIAL, R6 deferred to students; Phase T proposed)
...
| T | Public Release, Partnership, and Teaching Excellence — converts the 2026-09-07 cold audit into independent public-source, partner-demo, and student-distribution gates. Covers public documentation, measured repository treeshaking and disclosure review, application trust boundaries, a genuinely history-isolated cohort artifact, partner rehearsal, novice onboarding trials, and independent release verification. It **does not authorize publication, deployment, canonical quote mutation, or any student-owned R6 implementation**. | orchestrated gated cascade; T1–T4 may run in parallel after T0 in separate worktrees; T7 is independent review | **PROPOSED; T0 READY, T1–T7 BLOCKED by declared dependencies.** The safe next action is T0 only. | [`PHASE-T-PUBLIC-RELEASE-EXCELLENCE-CASCADE.md`](PHASE-T-PUBLIC-RELEASE-EXCELLENCE-CASCADE.md) |
