...
| --- | --- | --- | --- |
| **Pack** | `agentic/<name>/` with a `PACK.md` at its root — **mandatory**; no `PACK.md`, not a pack | ≥2 surfaces **inside `agentic/`** must move together (skill + script, rule + skill, agent + skill), **or** a CI path depends on it, **or** it is a multi-file harness | `report-steward/`, `ide-mcp/` |
| **Leaf rule** | `agentic/rules/<name>.md` | one always-on discipline; one landing; no companion script | `rules/ttod-editing.md` |
| **Leaf skill** | `agentic/skills/<name>/SKILL.md` | a single named workflow; it may *call* a standalone repo-level CLI (name it in the row below). Promote to a pack when companion code would live inside `agentic/` | `skills/public-docs-i18n/` |
...
| `ide-mcp/` | pack | student IDE MCP harness. Its `mcp.cursor.json` is mirrored to `.cursor/mcp.json` as an identical copy (JSON cannot redirect); `verify-ide-mcp.py` enforces that the two are semantically equal (parsed JSON) |
| `rules/`, `skills/` | containers | hold leaves only — never packs. Each entry inside is one leaf |
| `rules/ttod-editing.md` | leaf rule | strict YAML editing discipline |
| `skills/public-docs-i18n/` | leaf skill | bilingual public-docs authoring; its step 2 calls the standalone CLI `scripts/translate_public_docs.py`, which lives outside `agentic/` and is used by nothing else |
...
(redirects), `.cursor/mcp.json` (the identical-copy exception above), and
